...
/**
 * Mirrors Credo's canonical OID4VCI / OID4VP session state into the host
 * TypeORM rows. The application-level entities only capture `state` at
 * offer / request creation time; without this listener subsequent
 * transitions (AccessTokenRequested → CredentialsPartiallyIssued →
 * Completed; RequestUriRetrieved → ResponseVerified) never surface
 * through the `/oid4vc/...sessions` admin endpoints.
 *
 * Credo remains the source of truth — this service is read-mostly with
 * a single `UPDATE state` write per transition, keyed by the Credo
 * session id we already persist on each row.
 */
@Injectable()

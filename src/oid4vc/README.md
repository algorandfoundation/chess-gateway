...
├── trust-registry/credebl/         CREDEBL trust-registry bridge
...
surface, no on-chain anchor, and would prevent issuer discovery via the
trust registry.
...
The OID4VC subsystem uses a dedicated `pawn_oid4vc_issuer_approle`
provisioned by `vault/development-init.ts`. Its role/secret pair is
written to `oid4vc-role-and-secrets.json` and mirrored into `.env`:
...
| `OID4VC_VAULT_ROLE_ID`       | _(unset)_                                | AppRole id for `pawn_oid4vc_issuer_approle`                                       |
| `OID4VC_VAULT_SECRET_ID`     | _(unset)_                                | AppRole secret id                                                                 |
| `OID4VC_MANAGER_USER_ID`     | `VAULT_MANAGER_KEY` (default `manager`)  | Vault transit key name for the manager identity                                   |
| `CREDEBL_ENABLED`            | `false`                                  | Toggle the CREDEBL trust-registry bridge                                          |
| `CREDEBL_BASE_URL`           | _(unset)_                                | CREDEBL platform base URL                                                         |
| `CREDEBL_ORG_ID`             | _(unset)_                                | CREDEBL organisation id                                                           |
| `CREDEBL_API_KEY`            | _(unset)_                                | CREDEBL API key                                                                   |
| `CREDEBL_VERIFY_FAIL_CLOSED` | `true`                                   | Reject presentations whose issuer is not in CREDEBL (when enabled)                |

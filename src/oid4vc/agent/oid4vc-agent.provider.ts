...
export class Oid4vcAgentProvider implements OnModuleInit, OnModuleDestroy {
   ...
   * so verifiers (and the CREDEBL trust registry) can pin it.
   *
   ...
   *      transit key). Requires the OID4VC AppRole.
   */
  async ensureIssuerDid(): Promise<IssuerDid> {
    ...
    if (!this.tokenProvider.isConfigured()) {
      throw new Error(
        'Oid4vcAgentProvider: cannot derive the issuer did:algo because the OID4VC Vault ' +
    ...
    if (matching) {
      ...
      if (onChain) {
        await this.trustRegistry.registerIssuer(expectedDid);
    ...
    // Step 2 of the trust model (cf. src/oid4vc/TRUST_MODEL.md): now that
    // the manager DID is on chain, advertise it to the CREDEBL trust
    // registry. No-op when CREDEBL_ENABLED=false; failures are logged and
    // swallowed so a CREDEBL outage does not block credential issuance.
    await this.trustRegistry.registerIssuer(did);
    return result;
  }

  /**
   * Hard invariant: the OID4VC issuer DID **must** be a `did:algo`. The
   * issuer DID is the cryptographic root of trust we publish to CREDEBL

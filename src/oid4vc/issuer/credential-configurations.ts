 ...
 *
 * Kept in its own module to avoid an import cycle between
 * `Oid4vcIssuerService` (which depends on `Oid4vcAgentProvider`) and the
 * `CredeblTrustRegistryService` (which depends on this constant and is
 * itself wired into `Oid4vcAgentProvider`).
 */

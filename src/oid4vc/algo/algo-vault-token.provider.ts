 ...
 * service to Vault via AppRole credentials supplied through environment
 * variables — `OID4VC_VAULT_ROLE_ID` and `OID4VC_VAULT_SECRET_ID` — which is
 * the same mechanism the rest of the platform uses for machine-to-machine
 * Vault access.
 *
...
export class AlgoVaultTokenProvider {
  ...
  isConfigured(): boolean {
    return Boolean(
      this.configService.get<string>('OID4VC_VAULT_ROLE_ID') &&
        this.configService.get<string>('OID4VC_VAULT_SECRET_ID'),
    );
  }
  ...
  async getToken(): Promise<string> {
    if (this.cachedToken) {
      try {
        ...
        this.logger.warn(
          `Cached OID4VC Vault token rejected (${(err as Error).message}); re-authenticating via AppRole`,
        );
  ...
  private async login(): Promise<string> {
    const roleId = this.configService.get<string>('OID4VC_VAULT_ROLE_ID');
    const secretId = this.configService.get<string>('OID4VC_VAULT_SECRET_ID');
    if (!roleId || !secretId) {
      throw new Error(
        'OID4VC_VAULT_ROLE_ID and OID4VC_VAULT_SECRET_ID must be configured to use the did:algo registrar/issuer.',
      );
    }
    const token = await this.vaultService.getTokenWithRole(roleId, secretId);
    this.cachedToken = token;
    this.logger.log('Acquired OID4VC Vault token via AppRole login');
    return token;
  }
}

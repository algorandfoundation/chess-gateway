 ...
 * Ed25519 signing to HashiCorp Vault when the requested key has been
 * registered in {@link vaultSigningRegistry}. All other behaviour (key
 * generation for non-Vault keys, storage, verification, wallet lifecycle) is
 * inherited unchanged from `AskarWallet`.
 *
 ...
 *    releases — the abstraction is a poor fit for an external KMS.
 *
 * Why we override `sign` and not `createKey`:
 *  - The on-chain `did:algo` document is already created by
 *    {@link DidService} using the Vault public key; there is no need for
 *    Credo to materialise a new key. The registrar binds the published
 *    public key to its Vault transit name in the registry, and from then
 *    on every Credo signing call (issuer JWS, SD-JWT VC,
 *    verifier authorisation request, …) routes through Vault.
...
export class VaultAskarWallet extends AskarWallet {
  ...
  override async sign(options: WalletSignOptions): Promise<CredoBuffer> {
    ...
    // Only Ed25519 is delegated. Vault's transit engine in our deployment
    // only mints Ed25519 keys; if someone ever asks us to sign with a
    // different key type we want to fall through to Askar's native path.
    ...
    if (!binding) {
      // Not a Vault-bound key — let AskarWallet sign with whatever it has
      // locally. This keeps the door open for non-Vault keys (e.g. test
    ...
    if (Array.isArray(data)) {
      ...
      throw new WalletError(
        `VaultAskarWallet: multi-message signing is not supported for Vault-backed keys (publicKeyBase58=${key.publicKeyBase58}).`,
      );
    }
    ...
    try {
      ...
      if (bytes.length !== 64) {
        throw new WalletError(
          `VaultAskarWallet: Vault returned a ${bytes.length}-byte signature for ${key.publicKeyBase58}; expected 64 bytes.`,
        );
      }
      // Sanity check: the signature must verify against the requested
      // public key. This catches Vault key rotation drift and binding
      ...
      if (!verified) {
        throw new WalletError(
          `VaultAskarWallet: Vault signature did not verify against ${key.publicKeyBase58}; ` +
            `the binding may be stale or the Vault key has been rotated.`,
        );
      }
      ...
      throw new WalletError(
        `VaultAskarWallet: Vault signing failed for ${key.publicKeyBase58} (vaultKey=${binding.vaultKeyName}): ${(error as Error).message}`,
 ...
 */
export const publicKeyToBase58 = (publicKey: Uint8Array): string =>
  TypedArrayEncoder.toBase58(publicKey);

...
export class SponsoredService {
    ...
    /**
     * Claims assets from the authenticated user's vault-managed key to their linked wallet address.
     * Only the authenticated user can claim assets for themselves.
     * The sponsor (manager) covers all transaction fees via an atomic fee-bearing group.
     * @param betterAuthUserId - The authenticated user's BetterAuth ID (passed from session)
     */
    async claimAsset(
        ...
        if (!betterAuthUserId) {
            throw new ForbiddenException('User must be authenticated to claim assets.');
        }

...
describe('WalletService', () => {
  ...
  describe('sponsorTransactionGroup()', () => {
    ...
    const sponsorAddress = new AlgorandEncoder().encodeAddress(sponsorPublicKey);
    ...
    const buildPay = (from: string, to: string, amount: number, fee: number): Uint8Array => {
      const crafter = new AlgorandTransactionCrafter('test-genesis-id', 'test-genesis-hash');
      return crafter.pay(amount, from, to).addFee(fee).addFirstValidRound(1).addLastValidRound(1001).get().encode();
    };

    const toB64 = (tx: Uint8Array): string => Buffer.from(tx).toString('base64');

    const signUserTxn = (tx: Uint8Array): Uint8Array => {
      // Produce a signed-transaction msgpack envelope `{ txn, sig }` with a dummy signature.
      const encoder = new AlgorandEncoder();
      const decoded: any = encoder.decodeTransaction(tx);
      return encoder.encodeSignedTransaction({ txn: decoded, sig: new Uint8Array(64) });
    };

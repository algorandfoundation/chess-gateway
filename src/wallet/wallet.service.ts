...
export class WalletService {
  ...
  async getSponsorInfo(vault_token: string): Promise<SponsorDetailDto> {
    ...
    return {
      public_address: new AlgorandEncoder().encodeAddress(sponsorPublicKey),
    };
  }
  ...
  async sponsorTransactionGroup(
    ...
    const sponsorPublicKey: Buffer = await this.vaultService.getManagerPublicKey(vault_token);
    const sponsorAddress: string = new AlgorandEncoder().encodeAddress(sponsorPublicKey);
    const encoder = new AlgorandEncoder();

    // ---- Group ID validation ----
    const firstGroupBytes: Uint8Array | undefined = envelopes[0].txn.grp;
    ...
    const sponsorTxnSender = encoder.encodeAddress(Buffer.from(sponsorTxn.snd));
    ...
    if (!sponsorTxn.rcv || encoder.encodeAddress(Buffer.from(sponsorTxn.rcv)) !== sponsorAddress) {
      throw new Error(`Sponsor fee transaction receiver must be the sponsor address (${sponsorAddress})`);
    }
    const sponsorAmt: bigint = BigInt(sponsorTxn.amt ?? 0n);
    if (sponsorAmt !== 0n) {
    ...
    for (let i = 1; i < envelopes.length; i++) {
      ...
      const userFee: bigint = BigInt(env.txn.fee ?? 0n);
      if (userFee !== 0n) {
        throw new Error(`User transaction at index ${i} must have fee = 0`);
      }
      const userSender = encoder.encodeAddress(Buffer.from(env.txn.snd));
    ...
    const sponsorFee: bigint = BigInt(sponsorTxn.fee ?? 0n);
